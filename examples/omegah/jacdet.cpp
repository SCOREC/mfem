...
int main(int argc, char *argv[])
{
  ...
  oh::Mesh o_mesh(&lib);
  oh::binary::read(
      "/lore/joshia5/Meshes/curved/annulus-8.osh",
      //"/lore/joshia5/Meshes/curved/annulus3d-24.osh",
                    lib.world(), &o_mesh);
  ...
  else
  {
   ...
   const int geom_type = Geometry::TETRAHEDRON;

   int quad_order = 3;
   //const IntegrationRule *ir = nullptr;
   //IntegrationRules IntRulesLo(0, Quadrature1D::GaussLobatto);
   //ir = &IntRulesLo.Get(geom_type, quad_order); break;
   RT_FECollection ND_fec(quad_order, pmesh->Dimension());
   ParFiniteElementSpace ND_fes(pmesh, &ND_fec, pmesh->SpaceDimension());

   for (int i = 0; i < NE; i++)
   {
     const IntegrationRule *ir = Geometries.GetVertices(pmesh->GetElementGeometry(i));
     //printf("npts in ir %d\n", ir->GetNPoints());
     ElementTransformation *transf = ND_fes.GetElementTransformation(i);
     for (int j = 0; j < ir->GetNPoints(); j++)
     {
       transf->SetIntPoint(&ir->IntPoint(j));
       DenseMatrix jac = transf->Jacobian();
       //printf("computed J\n");
       double j_det = transf->Jacobian().Det();
       printf("j_det %f\n", j_det);
     }
   }
